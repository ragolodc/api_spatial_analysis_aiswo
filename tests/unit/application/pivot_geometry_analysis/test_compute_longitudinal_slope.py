...
def _spans_config(lengths: tuple[float, ...]) -> SpansConfig:
    return SpansConfig(
        [Spans(position=i + 1, length=l, dry_weight=0.0, service_weight=0.0) for i, l in enumerate(lengths)]
    )


SPANS = _spans_config((100.0, 100.0, 100.0))
...
def test_span_count():
    ...
    result = SVC.execute(uuid4(), [profile], SPANS, CONFIG)
...
def test_multiple_azimuths():
    ...
    result = SVC.execute(uuid4(), profiles, SPANS, CONFIG)
